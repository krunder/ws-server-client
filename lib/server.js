...
class Server {
  ...
  async start() {
    ...
    // Tell us we are now listening on the specified port
  ...
  async registerNamespaces() {
    ...
    Object.values(this.namespaces).forEach(namespace => {
      this._io.of(`/${namespace.path}`).on('connection', socket => {
        Object.values(this.events).filter(event => !!event.instance.namespace()).forEach(event => {
          socket.on(event.path, (payload, callback) => {
            callback(event.instance.listen(socket, payload));
          });
        });
